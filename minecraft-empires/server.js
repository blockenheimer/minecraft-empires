...
const CATALOG = {
 'Infantaria':{hp:1,dmg:1,rangeVis:2},'Tanque Pesado':{hp:75,dmg:45,rangeVis:2},'Tanque Médio':{hp:60,dmg:45,rangeVis:2},'Tanque Leve':{hp:40,dmg:25,rangeVis:2},'Anti-Air':{hp:20,dmg:25,rangeVis:2},'Caça-Tanque':{hp:50,dmg:65,rangeVis:2},'Artilharia Móvel':{hp:30,dmg:50,rangeVis:4},'Artilharia Pesada':{hp:60,dmg:60,rangeVis:4},'Engenharia de Combate':{hp:40,dmg:0,rangeVis:2},'Navio Pequeno (Patrulha)':{hp:100,dmg:30,rangeVis:3},'Submarino':{hp:200,dmg:60,rangeVis:3},'Battleship':{hp:1000,dmg:100,rangeVis:4},'Porta-Aviões':{hp:1000,dmg:40,rangeVis:4},'Navio Médio (Destroyer)':{hp:400,dmg:50,rangeVis:3},'Navio Carga/Transporte':{hp:200,dmg:10,rangeVis:2},'Desembarque Médio':{hp:200,dmg:15,rangeVis:2},'Desembarque Pequeno':{hp:100,dmg:10,rangeVis:2},'Desembarque Grande':{hp:200,dmg:20,rangeVis:2},'Avião Caça (Fighter)':{hp:40,dmg:45,rangeVis:5,airMaxTurns:4},'Attacker Médio/Pequeno':{hp:45,dmg:55,rangeVis:4,airMaxTurns:5},'Bombardeiro Médio':{hp:60,dmg:70,rangeVis:4,airMaxTurns:6},'Bombardeiro Grande':{hp:80,dmg:120,rangeVis:4,airMaxTurns:6}
};
...
function channelTeleportTarget(r,u,targetX,targetY){
  if(!u||!CATALOG[u.type])return null; const navals=new Set(['Navio Pequeno (Patrulha)','Submarino','Battleship','Porta-Aviões','Navio Médio (Destroyer)','Navio Carga/Transporte','Desembarque Médio','Desembarque Pequeno','Desembarque Grande']); if(!navals.has(u.type))return null;
...
function submarineDetectorRange(u){
  ...
  if(u.type==='Attacker Médio/Pequeno') return 1;
  if(u.type==='Bombardeiro Médio' || u.type==='Bombardeiro Grande') return 1;
  return 0;
}
...
function visibleState(r,p){
  ...
  // O Host tambem e a autoridade de simulacao da rodada. Ele precisa manter
  // a lista completa de unidades no estado local para conseguir executar o
  // movimento de tropas inimigas do S2 mesmo quando elas estiverem fora do
  // Fog of War visual. A renderizacao continua filtrando o que pode ser visto,
  // entao isso nao revela unidades na tela. Clientes convidados continuam
  // recebendo apenas a visao permitida.
  const stateUnits=p?.role==='host'
    ? ((Array.isArray(fs.units)?fs.units:[]).map(u=>({...u})))
    : filteredUnits(r,side);
  return {type:'state',config:publicConfig(r),sideId:p.sideId||null,countries:side?.countries||[],alliedCountries:coalitionCountries(r,side||{}),units:stateUnits,territoryOwners:fs.territoryOwners||r.setupTerritoryOwners||{},reconIntel:filteredIntel(r,side),log:filteredLog(r,side),fog:true,round:r.round,treaties:{telegrams:(r.treaties?.telegrams||[]).slice(-100),truce:r.treaties?.truce||null,peace:r.treaties?.peace||null,resumeReady:r.treaties?.resumeReady||{}},warReport:fs.warReport||null,channels:refreshChannelOwners(r).map(ch=>({...ch})),orders:r.ordersBySide?.[p.sideId]||{},authoritativeFullState:authoritativeResumeState(r,p)};
}
...
function startRound(r, advance=false){
  ...
  // Nova rodada: todas as unidades vivas voltam a ficar elegíveis para novas ordens.
  // O estado de bloqueio é específico da rodada anterior e não pode persistir.
  if(Array.isArray(r.fullState?.units)){
    r.fullState.units.forEach(u=>{
      const hp=Number(u.currentHp ?? u.hp ?? 0);
      if(hp>0){u.positionLocked=false;u.positionRound=r.round;}
    });
  }
...
function validateOrder(r,sideId,c){
  ...
  if(!u||!side.countries.includes(u.nation)||!['Guardar','Mover','Ataque','Reconhecimento'].includes(c.action))return false;
...
wss.on('connection',(ws)=>{
  ...
  (r.fullState.units||[]).forEach(u=>{u.positionLocked=false;u.positionRound=null;});
    ...
    if(m.type==='host_placement_done'&&player.role==='host'){
      ...
      r.fullState={units:Array.isArray(m.units)?m.units.map(normalizeUnitForServer).filter(Boolean):[],territoryOwners:chooseTerritoryOwners(m.territoryOwners,r.fullState?.territoryOwners,r.setupTerritoryOwners),reconIntel:{},log:Array.isArray(m.log)?m.log.slice(-160):[]};
    ...
    if(m.type==='guest_command'&&player.role==='guest'){
      ...
      if(c?.kind==='order'){
        ...
        u.targetX=Number(last?.x ?? c.targetX);
        u.targetY=Number(last?.y ?? c.targetY);
        toHost(r,{type:'remote_command',fromSideId:player.sideId,command:{...c,movePath:u.movePath,targetX:u.targetX,targetY:u.targetY}});
